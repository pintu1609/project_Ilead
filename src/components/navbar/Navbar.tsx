...
const Navbar = () => {
  ...
  return (
    <nav className={`navbar flex items-center h-[9rem] ${navbarBgColor} sticky z-50 w-full`}>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-0 left-0 w-full bg-black flex flex-col items-center gap-4 py-4 shadow-lg">
          <Link href="/home" className="text-lg text-white py-2 hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
            Home
          </Link>
          {links[0].links.map((link) =>
            link.name === "About Us" ? (
              <div key={link.url} className="flex flex-col items-center">
                <button
                  className="text-lg text-white py-2 hover:underline"
                  onClick={() => setIsAboutDropdownOpen(!isAboutDropdownOpen)}
                >
                  About Us
                </button>
                ...
                {isAboutDropdownOpen && (
                  <div className="flex flex-col gap-2">
                    <Link href="/about/components/company" className="text-lg text-white py-2 hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
                      Company
                    </Link>
                    <Link href="/about/components/team" className="text-lg text-white py-2 hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
                      Team
                    </Link>
                    <Link href="/about/components/careers" className="text-lg text-white py-2 hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
                      Careers
                    </Link>
                  </div>
                )}
              ...
              <Link key={link.url} href={`/${link.url}`} className="text-lg text-white py-2 hover:underline" onClick={() => setIsMobileMenuOpen(false)}>
                {link.name}
              </Link>
            )
          )}
          <Link href="/contact" passHref>
            <button className="mt-2 px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-600 focus:outline-none" onClick={() => setIsMobileMenuOpen(false)}>
              Contact us
            </button>
          </Link>
        </div>
